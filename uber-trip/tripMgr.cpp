...
void TripMgr::CreateTrip(Rider *pRider, Location *pSrcLoc, Location *pDstLoc) {
  TripMetaData *metaData =
      new TripMetaData(pSrcLoc, pDstLoc, pRider->getRating());
  StarategyMgr *starategyMgr = StarategyMgr::getStrategyMgr();
  PricingStrategy *pricingStrategy =
      starategyMgr->determinePricingStrategy(metaData);
  DriverMatchingStrategy *driverMatchingStrategy =
      starategyMgr->determineMatchingStrategy(metaData);

  Driver *driver = driverMatchingStrategy->matchDriver(metaData);
  double tripPrice = pricingStrategy->calculatePrice(metaData);

  Trip *trip = new Trip(pRider, driver, pSrcLoc, pDstLoc, tripPrice,
                        pricingStrategy, driverMatchingStrategy);
  int tripId = trip->getTripId();
  tripsInfo[tripId] = trip;
  tripsMetaDataInfo[tripId] = metaData;
}

unordered_map<int, Trip *> TripMgr::getTripsMap() { return tripsInfo; }