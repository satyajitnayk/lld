...
public:
  ...
  // register observer
  void addNotificationSender(string pOrderId, string pUserId,
                             INotificationSender *pNotificationSender) {
    if (find(notificationSendersMap[pOrderId].begin(),
             notificationSendersMap[pOrderId].end(),
             make_pair(pUserId, pNotificationSender)) !=
        notificationSendersMap[pOrderId].end()) {
      ////making sure the sender is already not there in the vector
      // if this check is not put, then multiple notifications will be sent by
      // same sender
      notificationSendersMap[pOrderId].push_back(
          make_pair(pUserId, pNotificationSender));
    }
  }
  ...
  void removeNotificationSender(string pOrderId, string pUserId,
                                INotificationSender *pNotificationSender) {
  ...
  // notify Observers
  void notify(string pOrderId, string pMessage) {
    for (auto &sender : notificationSendersMap[pOrderId]) {
      sender.second->sendNotification(sender.first, pMessage);
    }
  }
