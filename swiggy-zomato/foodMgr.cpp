...
void FoodMgr::addRestaurantForNotificationUpdates(string pOrderId,
                                                  string pRestaurantId) {
  NotificationMgr *notificationMgr = NotificationMgr::getNotificationMgr();
  // we can add push or whatsapp notifications in same way. Basically, we are
  // keeping all notifications customisable
  notificationMgr->addNotificationSender(pOrderId, pRestaurantId,
                                         new PushNotificationSender());
}

void FoodMgr::prepareFood(string pOrderId, string pRestaurantId,
  ...
  // The order in which the restaurant, user & delivery partner are added to the
  // notification mgr will decide which notifications they receive For
  // interviews, This is too much detailing though, we can just mention to
