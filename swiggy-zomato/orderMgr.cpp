...
void OrderMgr::addUserForNotificationUpdates(string pOrderId, Order *pOrder) {
  ...
  notificationMgr->addNotificationSender(pOrderId, pOrder->getUserId(),
...
void OrderMgr::manageDelivery(string pOrderId, Order *pOrder) {
  DeliveryMetaData *metaData = new DeliveryMetaData(
      pOrderId, pOrder->getUserLocation(), pOrder->getRestaurantLocation());
  deliveryMgr->manageDelivery(pOrderId, metaData);
}

void OrderMgr::manageFood(string pOrderId, Order *pOrder) {
  foodMgr->prepareFood(pOrderId, pOrder->getRestaurantId(),
                       pOrder->getDishes());
}
...
Order *OrderMgr::getOrder(string pOrderName) { return ordersMap[pOrderName]; }