...
int main() {
  ...
  Restaurant *chineseRest =
      new Restaurant("chinese vala", owner1, new Location(1, 2));
  ...
  Restaurant *southIndianRest =
      new Restaurant("south indian food", owner2, new Location(2, 3));
  ...
  // Note that restaurant owners can exist without restaurants, we have used
  // aggregation relationship This can lead to owners being present with no
  // restaurants and thus not added in restaurant manager This is how we have
  // designed and we should know consequences of the way we have structured.
  ...
  DeliveryPartnerMgr *deliveryPartnerMgr =
  ...
  // users can exist without usermgr as well. That's why it is an aggregation
  // relationship
  ...
  // I am passing same dish object that was created by res, for C++ folks - it
  // should be different dish object
  ...
  orderMgr->createOrder(
      "order1", order1); // Ideally, the id should be created in order manager
                         // when order is created
                         // This is just for simplicity purposes and has been
                         // mentioned in the class as well We have done same for
                         // all ids - user, restaurant, delivery partner etc.
