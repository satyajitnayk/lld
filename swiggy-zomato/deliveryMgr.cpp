...
// This function should be broken down into sub parts and every method should
// have one small respinsibility
void DeliveryMgr::manageDelivery(string pOrderId,
                                 DeliveryMetaData *pDeliveryMetaData) {
  ...
  for (auto deliveryPartner : deliverypartners) {
    notificationMgr->notifyParticularUser(deliveryPartner->getName(),
                                          "Delivery Request",
  ...
  notificationMgr->notify(pOrderId, "Delivery Partner " +
                                        assignedDeliveryPartner->getName() +
