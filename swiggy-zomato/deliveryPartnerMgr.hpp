...
public:
  ...
  void addDeliveryPartner(string pDeliveryPartnerName,
                          DeliveryPartner *pDeliveryPartner);
  DeliveryPartner *getDeliveryPartner(string pDeliveryPartnerName);

  // This getter is added so that delivery partner matching strategy can access
  // the map and select nearest delivery partners
