...
public:
  ...
  void performDelivery(string pOrderId, DeliveryMetaData *pDeliveryMetaData) {
    ...
    double restaurantLocLatitude =
        pDeliveryMetaData->getRestaurantLoc()->getLatitude();
    double restaurantLocLongitude =
        pDeliveryMetaData->getRestaurantLoc()->getLongitude();
    notificationMgr->notify(pOrderId,
                            name + " going to pick up delivery from location " +
                                to_string(restaurantLocLatitude) + "," +
                                to_string(restaurantLocLongitude));
    ...
    notificationMgr->notify(pOrderId, name + " reached the location " +
                                          to_string(userLocLatitude) + "," +
                                          to_string(userLocLongitude));

    this_thread::sleep_for(chrono::seconds(5));

    notificationMgr->notify(pOrderId,
                            name + " delivered the order. CONGRATULATIONS!!");
  }
};
